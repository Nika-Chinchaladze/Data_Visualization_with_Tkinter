...
class Visualization:
    def __init__(self, window):
        ...
        self.build_chart = Label(self.top_frame, text="Build Creative Dashboards", font=LABEL_FONT,
                                 fg="khaki", bg="light sea green")
        self.build_chart.place(x=500, y=2)

        # ================================ RIGHT FRAME ================================ #
        self.right_frame = Frame(self.window, bg="honeydew")
        self.right_frame.place(x=1065, y=45, width=300, height=645)

        # user information
        self.user_image = Image.open("user.png")
        used_photo = ImageTk.PhotoImage(self.user_image)
        self.human_image = Label(self.right_frame, image=used_photo, bg="honeydew", relief=RIDGE)
        self.human_image.image = used_photo
        self.human_image.place(x=100, y=50)

        self.user_name = Label(self.right_frame, text="Nika Chinchaladze", justify="center", font=USER_FONT,
                               bg="honeydew")
        self.user_name.place(x=65, y=180)

        # dashboard icon:
        self.dashboard_image = Image.open("dashboard.png")
        used_dashboard = ImageTk.PhotoImage(self.dashboard_image)
        self.dashboard_icon = Label(self.right_frame, image=used_dashboard, bg="honeydew", relief=RIDGE)
        self.dashboard_icon.image = used_dashboard
        self.dashboard_icon.place(x=20, y=260)

        self.dash_button = Button(self.right_frame, text="My Dashboard", justify="center", font=INFO_FONT,
                                  bg="honeydew", cursor="hand2", bd=0)
        self.dash_button.place(x=90, y=260)

        # manage icon:
        self.manage_image = Image.open("manage.png")
        used_manage = ImageTk.PhotoImage(self.manage_image)
        self.manage_icon = Label(self.right_frame, image=used_manage, bg="honeydew")
        self.manage_icon.image = used_manage
        self.manage_icon.place(x=32, y=300)

        self.manage_button = Button(self.right_frame, text="Manage Account", justify="center", font=INFO_FONT,
                                    bg="honeydew", cursor="hand2", bd=0)
        self.manage_button.place(x=90, y=305)

        # setting icon:
        self.setting_image = Image.open("setting.png")
        used_setting = ImageTk.PhotoImage(self.setting_image)
        self.setting_icon = Label(self.right_frame, image=used_setting, bg="honeydew")
        self.setting_icon.image = used_setting
        self.setting_icon.place(x=30, y=350)

        self.setting_button = Button(self.right_frame, text="Profile Settings", justify="center", font=INFO_FONT,
                                     bg="honeydew", cursor="hand2", bd=0)
        self.setting_button.place(x=90, y=353)

        # exit icon:
        self.exit_image = Image.open("exit.png")
        used_exit = ImageTk.PhotoImage(self.exit_image)
        self.exit_icon = Label(self.right_frame, image=used_exit, bg="honeydew")
        self.exit_icon.image = used_exit
        self.exit_icon.place(x=33, y=397)

        self.exit_button = Button(self.right_frame, text="Exit", justify="center", font=INFO_FONT,
                                  bg="honeydew", cursor="hand2", bd=0, command=self.close_window)
        self.exit_button.place(x=90, y=401)

        # error display label:
        self.error_info = Label(self.right_frame, text="", justify="center", font=INFO_FONT, bg="honeydew", bd=0)
        self.error_info.place(x=150, y=550)
